/**
 * Genera tabelle_riferimento_test.xlsx — tabelle percentili di riferimento
 * per tutti i test RankEX (Soccer + Personal Training).
 *
 * Uso: node scripts/generate-reference-tables.mjs
 */

import { resolve, dirname } from 'path'
import { fileURLToPath }    from 'url'
import { createRequire }    from 'module'

const require   = createRequire(import.meta.url)
const __dirname = dirname(fileURLToPath(import.meta.url))
const ROOT      = resolve(__dirname, '..')

const XLSX = require('xlsx')

// ── Tabelle percentili (fonte: src/utils/tables.js) ──────────────────────────

const TABLES = {

  // ── SOCCER YOUTH (7-9) ─────────────────────────────────────────────────────

  single_leg_stance: {
    label: 'Single Leg Stance', unit: 'secondi (somma 3 prove)', direction: 'direct',
    M: { '7-9': { 0:20, 5:25, 10:30, 20:38, 30:45, 40:50, 50:55, 60:60, 70:65, 80:77, 90:82, 95:87, 100:90 } },
    F: { '7-9': { 0:20, 5:25, 10:30, 20:38, 30:45, 40:50, 50:55, 60:60, 70:65, 80:77, 90:82, 95:87, 100:90 } },
  },
  sprint_10m: {
    label: 'Sprint 10m', unit: 'secondi', direction: 'inverse',
    M: {
      '7-9':   { 100:2.00, 95:2.20, 90:2.26, 80:2.39, 70:2.50, 60:2.60, 50:2.70, 40:2.82, 30:2.94, 20:3.10, 10:3.30, 5:3.40, 0:3.50 },
      '18-35': { 100:1.60, 95:1.75, 90:1.88, 80:2.00, 70:2.10, 60:2.20, 50:2.30, 40:2.45, 30:2.60, 20:2.80, 10:3.00, 5:3.20, 0:3.60 },
      '36-50': { 100:1.75, 95:1.90, 90:2.05, 80:2.18, 70:2.28, 60:2.38, 50:2.50, 40:2.65, 30:2.80, 20:3.00, 10:3.25, 5:3.45, 0:3.80 },
    },
    F: {
      '7-9':   { 100:2.00, 95:2.20, 90:2.26, 80:2.39, 70:2.50, 60:2.60, 50:2.70, 40:2.82, 30:2.94, 20:3.10, 10:3.30, 5:3.40, 0:3.50 },
      '18-35': { 100:1.80, 95:1.95, 90:2.10, 80:2.25, 70:2.38, 60:2.50, 50:2.65, 40:2.80, 30:2.95, 20:3.15, 10:3.40, 5:3.60, 0:4.00 },
      '36-50': { 100:1.95, 95:2.10, 90:2.25, 80:2.40, 70:2.55, 60:2.68, 50:2.82, 40:2.98, 30:3.15, 20:3.35, 10:3.60, 5:3.80, 0:4.20 },
    },
  },
  t_test_mini: {
    label: 'T-Test Mini (5m)', unit: 'secondi', direction: 'inverse',
    M: { '7-9': { 100:8.5, 95:10.0, 90:10.4, 80:11.1, 75:11.5, 70:11.8, 60:12.4, 50:13.0, 40:13.6, 30:14.2, 25:14.5, 20:15.0, 10:16.0, 5:16.5, 0:17.0 } },
    F: { '7-9': { 100:8.5, 95:10.0, 90:10.4, 80:11.1, 75:11.5, 70:11.8, 60:12.4, 50:13.0, 40:13.6, 30:14.2, 25:14.5, 20:15.0, 10:16.0, 5:16.5, 0:17.0 } },
  },
  shuttle_run_30m: {
    label: '30m Shuttle Run 6×5m', unit: 'secondi', direction: 'inverse',
    M: { '7-9': { 100:20.0, 95:22.5, 90:23.1, 80:24.4, 75:25.0, 70:25.6, 60:26.8, 50:28.0, 40:29.2, 30:30.4, 25:31.0, 20:32.2, 10:34.6, 5:35.8, 0:37.0 } },
    F: { '7-9': { 100:20.0, 95:22.5, 90:23.1, 80:24.4, 75:25.0, 70:25.6, 60:26.8, 50:28.0, 40:29.2, 30:30.4, 25:31.0, 20:32.2, 10:34.6, 5:35.8, 0:37.0 } },
  },

  // ── SOCCER JUNIOR (10-13) & SENIOR (14+) ────────────────────────────────────

  y_balance_anterior: {
    label: 'Y Balance Anterior Reach (%LL)', unit: '% lunghezza arto', direction: 'direct',
    M: { '10-13': { 0:40, 5:52, 10:56, 20:62, 30:66, 40:69, 50:72, 60:75, 70:78, 80:81, 90:85, 95:88, 100:100 } },
    F: { '10-13': { 0:40, 5:52, 10:56, 20:62, 30:66, 40:69, 50:72, 60:75, 70:78, 80:81, 90:85, 95:88, 100:100 } },
  },
  t_test_soccer_junior: {
    label: 'T-Test Standard (10m)', unit: 'secondi', direction: 'inverse',
    M: { '10-13': { 100:8.5, 95:9.8, 90:10.0, 80:10.25, 75:10.4, 70:10.6, 60:10.9, 50:11.2, 40:11.5, 30:11.8, 25:12.0, 20:12.4, 10:13.2, 5:13.6, 0:14.0 } },
    F: { '10-13': { 100:8.5, 95:9.8, 90:10.0, 80:10.25, 75:10.4, 70:10.6, 60:10.9, 50:11.2, 40:11.5, 30:11.8, 25:12.0, 20:12.4, 10:13.2, 5:13.6, 0:14.0 } },
  },
  six_minute_run: {
    label: '6 Minuti / Mezzo Cooper', unit: 'metri', direction: 'direct',
    M: { '10-13': { 0:700, 5:760, 10:820, 20:940, 30:1030, 40:1090, 50:1150, 60:1230, 70:1310, 80:1388, 90:1463, 95:1500, 100:1800 } },
    F: { '10-13': { 0:700, 5:760, 10:820, 20:940, 30:1030, 40:1090, 50:1150, 60:1230, 70:1310, 80:1388, 90:1463, 95:1500, 100:1800 } },
  },
  y_balance: {
    label: 'Y Balance Test (Composito %LL)', unit: '% lunghezza arto', direction: 'direct',
    M: {
      '10-11': { 0:67, 5:75, 10:78, 20:81, 30:83, 40:85, 50:87, 60:89, 70:91, 80:93, 90:96,  95:99,  100:107 },
      '12-13': { 0:71, 5:79, 10:82, 20:85, 30:87, 40:89, 50:91, 60:93, 70:95, 80:97, 90:100, 95:103, 100:111 },
      '14-15': { 0:73, 5:81, 10:84, 20:87, 30:89, 40:91, 50:93, 60:95, 70:97, 80:99, 90:102, 95:105, 100:113 },
      '16-17': { 0:77, 5:85, 10:88, 20:91, 30:93, 40:95, 50:97, 60:99, 70:101,80:103,90:106, 95:109, 100:117 },
      '18-40': { 0:65, 5:72, 10:78, 20:83, 30:87, 40:91, 50:96, 60:100,70:104,80:108,90:113, 95:117, 100:122 },
      '41-60': { 0:60, 5:67, 10:72, 20:78, 30:82, 40:86, 50:91, 60:95, 70:99, 80:103,90:108, 95:112, 100:117 },
    },
    F: {
      '10-11': { 0:71, 5:79, 10:82, 20:85, 30:87, 40:89, 50:91, 60:93, 70:95, 80:97, 90:100, 95:103, 100:111 },
      '12-13': { 0:69, 5:77, 10:80, 20:83, 30:85, 40:87, 50:89, 60:91, 70:93, 80:95, 90:98,  95:101, 100:109 },
      '14-15': { 0:71, 5:79, 10:82, 20:85, 30:87, 40:89, 50:91, 60:93, 70:95, 80:97, 90:100, 95:103, 100:111 },
      '16-17': { 0:75, 5:83, 10:86, 20:89, 30:91, 40:93, 50:95, 60:97, 70:99, 80:101,90:104, 95:107, 100:115 },
      '18-40': { 0:62, 5:70, 10:75, 20:80, 30:85, 40:89, 50:95, 60:99, 70:103,80:107,90:112, 95:116, 100:120 },
      '41-60': { 0:57, 5:64, 10:70, 20:75, 30:80, 40:84, 50:89, 60:93, 70:97, 80:101,90:106, 95:110, 100:115 },
    },
  },
  sprint_20m: {
    label: 'Sprint 20m', unit: 'secondi', direction: 'inverse',
    M: {
      '8-9':  { 100:4.13, 95:4.17, 90:4.21, 80:4.29, 70:4.37, 60:4.45, 50:4.53, 40:4.61, 30:4.69, 20:4.97, 10:5.46, 5:5.70, 0:5.95 },
      '10-11':{ 100:3.93, 95:3.96, 90:4.00, 80:4.07, 70:4.15, 60:4.24, 50:4.33, 40:4.41, 30:4.48, 20:4.70, 10:5.06, 5:5.24, 0:5.42 },
      '12-13':{ 100:3.70, 95:3.74, 90:3.77, 80:3.84, 70:3.91, 60:3.98, 50:4.06, 40:4.13, 30:4.20, 20:4.37, 10:4.64, 5:4.78, 0:4.91 },
      '14-15':{ 100:3.56, 95:3.58, 90:3.61, 80:3.66, 70:3.72, 60:3.78, 50:3.84, 40:3.90, 30:3.97, 20:4.12, 10:4.35, 5:4.47, 0:4.58 },
      '16-17':{ 100:3.47, 95:3.50, 90:3.52, 80:3.57, 70:3.62, 60:3.67, 50:3.72, 40:3.78, 30:3.83, 20:3.97, 10:4.18, 5:4.29, 0:4.39 },
      '18-35':{ 100:3.46, 95:3.48, 90:3.50, 80:3.54, 70:3.58, 60:3.63, 50:3.68, 40:3.73, 30:3.78, 20:3.91, 10:4.12, 5:4.23, 0:4.33 },
      '36-50':{ 100:3.50, 95:3.52, 90:3.54, 80:3.58, 70:3.62, 60:3.67, 50:3.72, 40:3.77, 30:3.82, 20:3.95, 10:4.16, 5:4.27, 0:4.37 },
    },
    F: {
      '8-9':  { 100:4.13, 95:4.17, 90:4.21, 80:4.29, 70:4.37, 60:4.45, 50:4.53, 40:4.61, 30:4.69, 20:4.97, 10:5.46, 5:5.70, 0:5.95 },
      '10-11':{ 100:3.93, 95:3.96, 90:4.00, 80:4.07, 70:4.15, 60:4.24, 50:4.33, 40:4.41, 30:4.48, 20:4.70, 10:5.06, 5:5.24, 0:5.42 },
      '12-13':{ 100:3.70, 95:3.74, 90:3.77, 80:3.84, 70:3.91, 60:3.98, 50:4.06, 40:4.13, 30:4.20, 20:4.37, 10:4.64, 5:4.78, 0:4.91 },
      '14-15':{ 100:3.56, 95:3.58, 90:3.61, 80:3.66, 70:3.72, 60:3.78, 50:3.84, 40:3.90, 30:3.97, 20:4.12, 10:4.35, 5:4.47, 0:4.58 },
      '16-17':{ 100:3.47, 95:3.50, 90:3.52, 80:3.57, 70:3.62, 60:3.67, 50:3.72, 40:3.78, 30:3.83, 20:3.97, 10:4.18, 5:4.29, 0:4.39 },
      '18-35':{ 100:3.96, 95:3.98, 90:4.00, 80:4.04, 70:4.08, 60:4.13, 50:4.18, 40:4.23, 30:4.28, 20:4.41, 10:4.62, 5:4.73, 0:4.83 },
      '36-50':{ 100:4.05, 95:4.07, 90:4.09, 80:4.13, 70:4.17, 60:4.22, 50:4.27, 40:4.32, 30:4.37, 20:4.50, 10:4.71, 5:4.82, 0:4.92 },
    },
  },
  '505_cod_agility': {
    label: '505 COD Agility', unit: 'secondi', direction: 'inverse',
    M: {
      '10-11':{ 100:2.15, 95:2.25, 90:2.37, 80:2.50, 70:2.59, 60:2.65, 50:2.70, 40:2.76, 30:2.82, 20:2.90, 10:3.02, 5:3.13, 0:3.40 },
      '12-13':{ 100:2.06, 95:2.14, 90:2.27, 80:2.38, 70:2.47, 60:2.54, 50:2.60, 40:2.66, 30:2.73, 20:2.82, 10:2.93, 5:3.03, 0:3.28 },
      '14-15':{ 100:1.97, 95:2.06, 90:2.16, 80:2.26, 70:2.35, 60:2.43, 50:2.50, 40:2.56, 30:2.62, 20:2.70, 10:2.82, 5:2.91, 0:3.15 },
      '16-17':{ 100:1.94, 95:2.02, 90:2.09, 80:2.17, 70:2.24, 60:2.32, 50:2.40, 40:2.47, 30:2.55, 20:2.62, 10:2.73, 5:2.85, 0:3.09 },
      '18-35':{ 100:1.95, 95:2.03, 90:2.10, 80:2.19, 70:2.27, 60:2.33, 50:2.40, 40:2.48, 30:2.57, 20:2.68, 10:2.80, 5:2.91, 0:3.15 },
      '36-50':{ 100:2.07, 95:2.15, 90:2.23, 80:2.33, 70:2.42, 60:2.50, 50:2.58, 40:2.67, 30:2.78, 20:2.92, 10:3.08, 5:3.23, 0:3.45 },
    },
    F: {
      '10-11':{ 100:2.15, 95:2.25, 90:2.37, 80:2.50, 70:2.59, 60:2.65, 50:2.70, 40:2.76, 30:2.82, 20:2.90, 10:3.02, 5:3.13, 0:3.40 },
      '12-13':{ 100:2.06, 95:2.14, 90:2.27, 80:2.38, 70:2.47, 60:2.54, 50:2.60, 40:2.66, 30:2.73, 20:2.82, 10:2.93, 5:3.03, 0:3.28 },
      '14-15':{ 100:1.97, 95:2.06, 90:2.16, 80:2.26, 70:2.35, 60:2.43, 50:2.50, 40:2.56, 30:2.62, 20:2.70, 10:2.82, 5:2.91, 0:3.15 },
      '16-17':{ 100:1.94, 95:2.02, 90:2.09, 80:2.17, 70:2.24, 60:2.32, 50:2.40, 40:2.47, 30:2.55, 20:2.62, 10:2.73, 5:2.85, 0:3.09 },
      '18-35':{ 100:2.15, 95:2.25, 90:2.35, 80:2.45, 70:2.55, 60:2.65, 50:2.75, 40:2.87, 30:2.99, 20:3.15, 10:3.33, 5:3.50, 0:3.85 },
      '36-50':{ 100:2.30, 95:2.42, 90:2.53, 80:2.65, 70:2.77, 60:2.89, 50:3.01, 40:3.15, 30:3.30, 20:3.47, 10:3.67, 5:3.85, 0:4.25 },
    },
  },
  beep_test: {
    label: 'Beep Test / MSFT', unit: 'livello', direction: 'direct',
    M: {
      '8-9':  { 0:2.0, 5:2.2, 10:2.6, 20:3.5, 30:4.0, 40:4.5, 50:5.0, 60:5.5, 70:6.0, 80:6.5, 90:7.3, 95:7.7, 100:8.5 },
      '10-11':{ 0:2.0, 5:2.2, 10:2.7, 20:3.7, 30:4.2, 40:4.7, 50:5.3, 60:5.8, 70:6.4, 80:6.9, 90:7.9, 95:8.4, 100:9.2 },
      '12-13':{ 0:2.0, 5:2.3, 10:2.9, 20:4.2, 30:4.9, 40:5.5, 50:6.2, 60:6.8, 70:7.5, 80:8.1, 90:9.2, 95:9.7, 100:10.5 },
      '14-15':{ 0:2.2, 5:2.7, 10:3.4, 20:4.7, 30:5.5, 40:6.3, 50:7.0, 60:7.7, 70:8.5, 80:9.2, 90:10.5, 95:11.1, 100:12.0 },
      '16-17':{ 0:2.5, 5:3.0, 10:3.7, 20:5.2, 30:6.0, 40:6.8, 50:7.6, 60:8.3, 70:9.1, 80:9.9, 90:11.3, 95:12.0, 100:13.0 },
      '18-35':{ 0:4.0, 5:5.5, 10:6.5, 20:7.5, 30:8.5, 40:9.5, 50:10.5, 60:11.5, 70:12.5, 80:13.5, 90:14.5, 95:15.5, 100:17.0 },
      '36-50':{ 0:3.0, 5:4.5, 10:5.5, 20:6.5, 30:7.5, 40:8.5, 50:9.5, 60:10.5, 70:11.5, 80:12.5, 90:13.5, 95:14.5, 100:16.0 },
    },
    F: {
      '8-9':  { 0:1.8, 5:2.2, 10:2.6, 20:3.2, 30:3.7, 40:4.1, 50:4.4, 60:4.7, 70:5.1, 80:5.5, 90:6.1, 95:6.6, 100:7.2 },
      '10-11':{ 0:1.5, 5:1.7, 10:2.2, 20:3.2, 30:3.7, 40:4.1, 50:4.5, 60:4.9, 70:5.4, 80:5.8, 90:6.6, 95:7.0, 100:7.6 },
      '12-13':{ 0:1.3, 5:1.5, 10:2.1, 20:3.2, 30:3.7, 40:4.2, 50:4.7, 60:5.2, 70:5.7, 80:6.2, 90:7.0, 95:7.4, 100:8.0 },
      '14-15':{ 0:1.3, 5:1.5, 10:2.1, 20:3.2, 30:3.8, 40:4.3, 50:4.8, 60:5.3, 70:5.9, 80:6.4, 90:7.3, 95:7.7, 100:8.4 },
      '16-17':{ 0:1.3, 5:1.5, 10:2.1, 20:3.2, 30:3.8, 40:4.3, 50:4.9, 60:5.4, 70:6.0, 80:6.5, 90:7.4, 95:7.9, 100:8.6 },
      '18-35':{ 0:3.0, 5:4.5, 10:5.5, 20:6.5, 30:7.5, 40:8.5, 50:9.5, 60:10.5, 70:11.5, 80:12.5, 90:13.5, 95:14.0, 100:15.5 },
      '36-50':{ 0:2.0, 5:3.5, 10:4.5, 20:5.5, 30:6.5, 40:7.5, 50:8.5, 60:9.5,  70:10.5, 80:11.5, 90:12.5, 95:13.0, 100:14.5 },
    },
  },

  // ── STANDING LONG JUMP (tutte le fasce) ─────────────────────────────────────

  standing_long_jump: {
    label: 'Standing Long Jump', unit: 'cm', direction: 'direct',
    M: {
      '7-9':   { 0:45,  5:51,  10:57,  20:69,  25:75,  30:79,  40:87,  50:95,  60:103, 70:111, 75:115, 80:119, 90:126, 95:130, 100:145 },
      '9-10':  { 0:96,  5:102, 10:108, 20:120, 30:125, 40:131, 50:138, 60:143, 70:150, 80:160, 90:176, 95:184, 100:193 },
      '11-12': { 0:98,  5:108, 10:117, 20:137, 30:143, 40:148, 50:153, 60:156, 70:164, 80:168, 90:178, 95:183, 100:187 },
      '13-14': { 0:121, 5:127, 10:133, 20:145, 30:153, 40:160, 50:167, 60:177, 70:181, 80:188, 90:197, 95:202, 100:206 },
      '15-16': { 0:121, 5:128, 10:134, 20:147, 30:159, 40:169, 50:175, 60:183, 70:191, 80:201, 90:211, 95:216, 100:221 },
      '17-18': { 0:135, 5:143, 10:150, 20:165, 30:175, 40:185, 50:195, 60:200, 70:210, 80:216, 90:228, 95:234, 100:240 },
      '18-29': { 0:190, 5:194, 10:198, 20:206, 30:214, 40:222, 50:230, 60:236, 70:242, 80:250, 90:260, 95:265, 100:270 },
      '30-39': { 0:185, 5:189, 10:193, 20:201, 30:209, 40:217, 50:225, 60:231, 70:237, 80:245, 90:255, 95:260, 100:265 },
      '40-49': { 0:180, 5:183, 10:186, 20:192, 30:198, 40:204, 50:210, 60:218, 70:226, 80:235, 90:245, 95:250, 100:255 },
      '50-59': { 0:170, 5:173, 10:176, 20:182, 30:188, 40:194, 50:200, 60:206, 70:212, 80:220, 90:230, 95:235, 100:240 },
      '60-69': { 0:160, 5:163, 10:166, 20:172, 30:178, 40:184, 50:190, 60:196, 70:202, 80:210, 90:220, 95:225, 100:230 },
      '70-79': { 0:145, 5:148, 10:151, 20:157, 30:163, 40:169, 50:175, 60:181, 70:187, 80:195, 90:205, 95:210, 100:215 },
    },
    F: {
      '7-9':   { 0:45,  5:51,  10:57,  20:69,  25:75,  30:79,  40:87,  50:95,  60:103, 70:111, 75:115, 80:119, 90:126, 95:130, 100:145 },
      '9-10':  { 0:96,  5:101, 10:105, 20:114, 30:120, 40:125, 50:130, 60:139, 70:145, 80:153, 90:165, 95:171, 100:177 },
      '11-12': { 0:90,  5:98,  10:106, 20:122, 30:140, 40:144, 50:152, 60:155, 70:160, 80:165, 90:175, 95:180, 100:185 },
      '13-14': { 0:109, 5:113, 10:116, 20:123, 30:128, 40:133, 50:140, 60:147, 70:153, 80:160, 90:173, 95:180, 100:186 },
      '15-16': { 0:96,  5:102, 10:107, 20:118, 30:124, 40:131, 50:135, 60:143, 70:149, 80:158, 90:170, 95:176, 100:182 },
      '17-18': { 0:82,  5:89,  10:95,  20:108, 30:112, 40:120, 50:126, 60:134, 70:140, 80:152, 90:157, 95:160, 100:162 },
      '18-29': { 0:130, 5:134, 10:138, 20:146, 30:154, 40:162, 50:170, 60:178, 70:186, 80:195, 90:205, 95:210, 100:215 },
      '30-39': { 0:125, 5:129, 10:133, 20:141, 30:149, 40:157, 50:165, 60:171, 70:177, 80:185, 90:195, 95:200, 100:205 },
      '40-49': { 0:131, 5:134, 10:136, 20:141, 30:145, 40:150, 50:155, 60:161, 70:167, 80:175, 90:180, 95:185, 100:190 },
      '50-59': { 0:124, 5:127, 10:129, 20:134, 30:138, 40:143, 50:147, 60:151, 70:156, 80:163, 90:169, 95:174, 100:179 },
      '60-69': { 0:117, 5:120, 10:122, 20:127, 30:131, 40:136, 50:140, 60:144, 70:149, 80:156, 90:162, 95:167, 100:172 },
      '70-79': { 0:106, 5:109, 10:111, 20:116, 30:120, 40:125, 50:129, 60:133, 70:138, 80:145, 90:151, 95:156, 100:161 },
    },
  },

  // ── PERSONAL TRAINING ───────────────────────────────────────────────────────

  sit_and_reach: {
    label: 'Sit and Reach', unit: 'cm', direction: 'direct',
    M: {
      '18-35': { 0:7,  5:10, 10:13, 20:16, 30:19, 40:22, 50:25, 60:28, 70:31, 80:34, 90:37, 95:40, 100:43 },
      '36-45': { 0:5,  5:8,  10:11, 20:14, 30:17, 40:20, 50:23, 60:26, 70:29, 80:32, 90:35, 95:38, 100:41 },
      '46-55': { 0:3,  5:6,  10:9,  20:12, 30:15, 40:18, 50:21, 60:24, 70:27, 80:30, 90:33, 95:35, 100:37 },
      '56-65': { 0:0,  5:3,  10:6,  20:9,  30:12, 40:15, 50:18, 60:21, 70:24, 80:27, 90:30, 95:32, 100:34 },
      '66+':   { 0:-3, 5:0,  10:3,  20:6,  30:9,  40:12, 50:15, 60:18, 70:21, 80:23, 90:26, 95:28, 100:30 },
    },
    F: {
      '18-35': { 0:12, 5:15, 10:18, 20:21, 30:24, 40:27, 50:30, 60:33, 70:36, 80:39, 90:42, 95:45, 100:48 },
      '36-45': { 0:9,  5:12, 10:15, 20:18, 30:21, 40:24, 50:27, 60:30, 70:33, 80:36, 90:39, 95:42, 100:45 },
      '46-55': { 0:6,  5:9,  10:12, 20:15, 30:18, 40:21, 50:24, 60:27, 70:30, 80:33, 90:36, 95:39, 100:42 },
      '56-65': { 0:3,  5:6,  10:9,  20:12, 30:15, 40:18, 50:21, 60:24, 70:27, 80:30, 90:33, 95:36, 100:39 },
      '66+':   { 0:-1, 5:2,  10:5,  20:8,  30:11, 40:14, 50:17, 60:20, 70:23, 80:26, 90:29, 95:32, 100:35 },
    },
  },
  flamingo_test: {
    label: 'Flamingo Test', unit: 'n° cadute / 60s', direction: 'inverse',
    M: {
      '18-35': { 100:0, 95:1, 90:2, 80:3, 70:4, 60:5, 50:6, 40:7, 30:9,  20:11, 10:14, 5:17, 0:20 },
      '36-45': { 100:1, 95:2, 90:3, 80:4, 70:5, 60:6, 50:7, 40:9, 30:11, 20:13, 10:16, 5:20, 0:24 },
      '46-55': { 100:2, 95:3, 90:4, 80:5, 70:6, 60:7, 50:9, 40:11,30:13, 20:16, 10:19, 5:23, 0:27 },
      '56-65': { 100:3, 95:4, 90:5, 80:6, 70:7, 60:9, 50:11,40:13,30:16, 20:19, 10:23, 5:27, 0:31 },
      '66+':   { 100:5, 95:6, 90:7, 80:9, 70:10,60:12,50:14,40:16,30:19, 20:23, 10:27, 5:32, 0:37 },
    },
    F: {
      '18-35': { 100:0, 95:1, 90:2, 80:3, 70:4, 60:5, 50:6, 40:7, 30:8,  20:10, 10:12, 5:15, 0:18 },
      '36-45': { 100:1, 95:2, 90:3, 80:4, 70:5, 60:6, 50:7, 40:8, 30:10, 20:12, 10:14, 5:17, 0:20 },
      '46-55': { 100:2, 95:3, 90:4, 80:5, 70:6, 60:7, 50:8, 40:10,30:12, 20:14, 10:17, 5:20, 0:23 },
      '56-65': { 100:3, 95:4, 90:5, 80:6, 70:7, 60:8, 50:10,40:12,30:14, 20:17, 10:20, 5:24, 0:28 },
      '66+':   { 100:4, 95:5, 90:6, 80:8, 70:9, 60:11,50:13,40:15,30:17, 20:20, 10:24, 5:29, 0:34 },
    },
  },
  ymca_step_test: {
    label: 'YMCA Step Test', unit: 'bpm (post-esercizio)', direction: 'inverse',
    M: {
      '18-35': { 100:77,  95:81,  90:85,  75:103, 50:120, 30:123, 15:127, 5:136, 0:140 },
      '36-45': { 100:80,  95:84,  90:88,  75:102, 50:120, 30:125, 15:129, 5:138, 0:142 },
      '46+':   { 100:81,  95:90,  90:99,  75:111, 50:120, 30:124, 15:130, 5:138, 0:142 },
    },
    F: {
      '18-35': { 100:64,  95:79,  90:94,  75:109, 50:118, 30:122, 15:129, 5:137, 0:141 },
      '36-45': { 100:68,  95:79,  90:90,  75:106, 50:118, 30:125, 15:134, 5:145, 0:150 },
      '46+':   { 100:71,  95:84,  90:97,  75:108, 50:118, 30:124, 15:130, 5:145, 0:152 },
    },
  },
  dinamometro_hand_grip: {
    label: 'Dinamometro Hand Grip', unit: 'kg', direction: 'direct',
    M: {
      '18-35': { 0:31, 5:34, 10:37, 20:40, 30:43, 40:46, 50:48, 60:50, 70:53, 80:56, 90:60, 95:64, 100:68 },
      '36-45': { 0:29, 5:32, 10:35, 20:38, 30:41, 40:43, 50:46, 60:48, 70:51, 80:54, 90:58, 95:62, 100:66 },
      '46-55': { 0:27, 5:30, 10:33, 20:35, 30:38, 40:40, 50:42, 60:45, 70:47, 80:50, 90:54, 95:58, 100:62 },
      '56-65': { 0:25, 5:27, 10:29, 20:31, 30:33, 40:35, 50:37, 60:39, 70:42, 80:45, 90:48, 95:52, 100:56 },
      '66+':   { 0:19, 5:21, 10:23, 20:25, 30:27, 40:29, 50:31, 60:33, 70:35, 80:38, 90:42, 95:46, 100:50 },
    },
    F: {
      '18-35': { 0:16, 5:18, 10:20, 20:22, 30:24, 40:26, 50:27, 60:28, 70:29, 80:31, 90:34, 95:36, 100:38 },
      '36-45': { 0:15, 5:17, 10:19, 20:21, 30:23, 40:24, 50:25, 60:26, 70:28, 80:30, 90:32, 95:34, 100:36 },
      '46-55': { 0:14, 5:16, 10:18, 20:20, 30:22, 40:23, 50:24, 60:25, 70:26, 80:28, 90:30, 95:32, 100:34 },
      '56-65': { 0:13, 5:15, 10:17, 20:19, 30:20, 40:21, 50:22, 60:23, 70:24, 80:25, 90:27, 95:29, 100:31 },
      '66+':   { 0:11, 5:13, 10:15, 20:16, 30:17, 40:18, 50:19, 60:20, 70:21, 80:22, 90:24, 95:25, 100:26 },
    },
  },
  sit_to_stand: {
    label: '5 Volte Alzati/Siediti', unit: 'secondi', direction: 'inverse',
    M: {
      '18-35': { 100:5.0, 95:5.5,  90:6.0,  80:6.5,  70:7.0,  60:7.4,  50:7.8,  40:8.3,  30:8.9,  20:9.6,  10:10.7, 5:11.7, 0:12.7 },
      '36-45': { 100:5.3, 95:5.8,  90:6.3,  80:6.9,  70:7.4,  60:7.8,  50:8.3,  40:8.9,  30:9.6,  20:10.5, 10:11.6, 5:12.8, 0:14.0 },
      '46-55': { 100:5.6, 95:6.2,  90:6.8,  80:7.5,  70:8.0,  60:8.5,  50:9.0,  40:9.7,  30:10.4, 20:11.3, 10:12.6, 5:14.0, 0:15.4 },
      '56-65': { 100:6.2, 95:6.8,  90:7.4,  80:8.0,  70:8.6,  60:9.2,  50:9.8,  40:10.5, 30:11.3, 20:12.2, 10:13.8, 5:15.4, 0:17.0 },
      '66+':   { 100:7.2, 95:7.8,  90:8.4,  80:9.0,  70:9.7,  60:10.4, 50:11.2, 40:12.0, 30:13.0, 20:14.1, 10:15.6, 5:17.2, 0:18.8 },
    },
    F: {
      '18-35': { 100:5.1, 95:5.6,  90:6.1,  80:6.6,  70:7.1,  60:7.6,  50:8.1,  40:8.6,  30:9.2,  20:10.0, 10:11.2, 5:12.5, 0:13.8 },
      '36-45': { 100:5.5, 95:6.0,  90:6.5,  80:7.1,  70:7.6,  60:8.1,  50:8.6,  40:9.2,  30:9.9,  20:10.8, 10:12.1, 5:13.5, 0:14.9 },
      '46-55': { 100:5.8, 95:6.4,  90:7.0,  80:7.6,  70:8.2,  60:8.8,  50:9.4,  40:10.0, 30:10.8, 20:11.7, 10:13.2, 5:14.7, 0:16.2 },
      '56-65': { 100:6.4, 95:7.0,  90:7.6,  80:8.3,  70:8.9,  60:9.5,  50:10.2, 40:10.9, 30:11.8, 20:12.9, 10:14.6, 5:16.3, 0:18.0 },
      '66+':   { 100:7.4, 95:8.0,  90:8.6,  80:9.3,  70:10.0, 60:10.8, 50:11.6, 40:12.5, 30:13.5, 20:14.8, 10:16.5, 5:18.5, 0:20.5 },
    },
  },
  drop_jump_rsi: {
    label: 'Drop Jump RSI', unit: 'altezza / tempo contatto', direction: 'direct',
    M: {
      '18-35': { 0:0.5, 5:0.8, 10:1.0, 20:1.3, 30:1.6, 40:1.9, 50:2.1, 60:2.3, 70:2.5, 80:2.8, 90:3.1, 95:3.4, 100:3.8 },
      '36-50': { 0:0.4, 5:0.7, 10:0.9, 20:1.1, 30:1.4, 40:1.6, 50:1.8, 60:2.0, 70:2.2, 80:2.5, 90:2.8, 95:3.0, 100:3.4 },
    },
    F: {
      '18-35': { 0:0.4, 5:0.6, 10:0.8, 20:1.0, 30:1.2, 40:1.4, 50:1.6, 60:1.8, 70:2.0, 80:2.2, 90:2.5, 95:2.7, 100:3.0 },
      '36-50': { 0:0.3, 5:0.5, 10:0.7, 20:0.9, 30:1.1, 40:1.3, 50:1.5, 60:1.7, 70:1.9, 80:2.1, 90:2.3, 95:2.5, 100:2.8 },
    },
  },
  t_test_agility: {
    label: 'T-Test Agility', unit: 'secondi', direction: 'inverse',
    M: { '18-40': { 100:8.5, 95:9.0, 90:9.5, 80:10.0, 70:10.3, 60:10.7, 50:11.0, 40:11.4, 30:11.8, 20:12.3, 10:13.0, 5:13.8, 0:15.0 } },
    F: { '18-40': { 100:9.5, 95:10.0, 90:10.5, 80:11.0, 70:11.3, 60:11.7, 50:12.0, 40:12.4, 30:12.8, 20:13.4, 10:14.2, 5:15.0, 0:16.5 } },
  },
  yo_yo_ir1: {
    label: 'Yo-Yo IR1', unit: 'metri', direction: 'direct',
    M: {
      '18-35': { 0:160,  5:320,  10:480,  20:720,  30:960,  40:1200, 50:1520, 60:1840, 70:2120, 80:2400, 90:2880, 95:3200, 100:3600 },
      '36-50': { 0:120,  5:240,  10:360,  20:560,  30:760,  40:1000, 50:1240, 60:1520, 70:1800, 80:2080, 90:2480, 95:2800, 100:3200 },
    },
    F: {
      '18-35': { 0:80,   5:160,  10:280,  20:400,  30:560,  40:720,  50:920,  60:1120, 70:1360, 80:1600, 90:1920, 95:2240, 100:2640 },
      '36-50': { 0:60,   5:120,  10:200,  20:320,  30:440,  40:600,  50:760,  60:960,  70:1200, 80:1440, 90:1720, 95:2000, 100:2400 },
    },
  },
  cmj: {
    label: 'CMJ (Counter Movement Jump)', unit: 'cm', direction: 'direct',
    M: {
      '18-35': { 0:12, 5:18, 10:22, 20:27, 30:32, 40:36, 50:40, 60:44, 70:48, 80:53, 90:58, 95:63, 100:70 },
      '36-50': { 0:10, 5:15, 10:18, 20:23, 30:27, 40:31, 50:35, 60:39, 70:43, 80:47, 90:52, 95:57, 100:63 },
    },
    F: {
      '18-35': { 0:8,  5:12, 10:15, 20:19, 30:23, 40:26, 50:29, 60:32, 70:36, 80:40, 90:45, 95:49, 100:55 },
      '36-50': { 0:6,  5:10, 10:13, 20:16, 30:20, 40:23, 50:26, 60:29, 70:33, 80:37, 90:41, 95:45, 100:50 },
    },
  },
}

// ── Definizione fogli ─────────────────────────────────────────────────────────

const SHEETS = [
  {
    name: 'Soccer Pulcini (7-9)',
    tests: ['single_leg_stance', 'sprint_10m', 't_test_mini', 'shuttle_run_30m', 'standing_long_jump'],
    ageFocus: ['7-9'],
  },
  {
    name: 'Soccer Junior (10-13)',
    tests: ['y_balance_anterior', 'sprint_20m', 't_test_soccer_junior', 'standing_long_jump', 'six_minute_run'],
    ageFocus: ['10-11', '12-13', '10-13', '9-10', '11-12', '13-14'],
  },
  {
    name: 'Soccer Senior (14+)',
    tests: ['y_balance', 'sprint_20m', '505_cod_agility', 'standing_long_jump', 'beep_test'],
    ageFocus: ['14-15', '16-17', '18-40', '18-35', '13-14', '15-16', '17-18', '18-29'],
  },
  {
    name: 'PT Health',
    tests: ['sit_and_reach', 'flamingo_test', 'ymca_step_test', 'dinamometro_hand_grip', 'sit_to_stand'],
    ageFocus: null,
  },
  {
    name: 'PT Active',
    tests: ['y_balance', 'dinamometro_hand_grip', 'ymca_step_test', 'standing_long_jump', 'sprint_10m'],
    ageFocus: ['18-40', '41-60', '18-35', '36-50', '18-35', '36-45', '18-29', '30-39'],
  },
  {
    name: 'PT Athlete',
    tests: ['drop_jump_rsi', 't_test_agility', 'yo_yo_ir1', 'sprint_20m', 'cmj'],
    ageFocus: ['18-35', '18-40', '36-50'],
  },
]

// ── Builder fogli ─────────────────────────────────────────────────────────────

function buildSheet(sheetDef) {
  const rows = []

  rows.push([`RankEX — Tabelle Percentili di Riferimento: ${sheetDef.name}`])
  rows.push([`Generato il ${new Date().toLocaleDateString('it-IT')}`])
  rows.push([])

  for (const testKey of sheetDef.tests) {
    const tbl = TABLES[testKey]
    if (!tbl) continue

    const dirLabel = tbl.direction === 'direct'
      ? 'DIRETTA — valore più alto = percentile più alto'
      : 'INVERSA — valore più basso = percentile più alto'

    rows.push([`▶  ${tbl.label}   (${tbl.unit})   ${dirLabel}`])

    // Raccoglie fascie da mostrare
    const allGroupsM = Object.keys(tbl.M ?? {})
    const allGroupsF = Object.keys(tbl.F ?? {})
    const allGroups  = [...new Set([...allGroupsM, ...allGroupsF])]

    // Filtra per ageFocus se definito
    const groups = sheetDef.ageFocus
      ? allGroups.filter(g => sheetDef.ageFocus.includes(g))
      : allGroups

    if (groups.length === 0) {
      // Nessuna fascia filtrata: usa tutto
      groups.push(...allGroups)
    }

    if (groups.length === 0) { rows.push([]); continue }

    // Header: Percentile | [fascia M] | [fascia F] | ...
    const header = ['Percentile']
    for (const g of groups) {
      header.push(`${g} M`)
      header.push(`${g} F`)
    }
    rows.push(header)

    // Raccoglie tutte le chiavi percentile presenti
    const allKeys = new Set()
    groups.forEach(g => {
      Object.keys(tbl.M?.[g] ?? {}).forEach(k => allKeys.add(Number(k)))
      Object.keys(tbl.F?.[g] ?? {}).forEach(k => allKeys.add(Number(k)))
    })

    // Ordina da 100 a 0
    const keys = [...allKeys].sort((a, b) => b - a)

    for (const k of keys) {
      const row = [k]
      for (const g of groups) {
        const mVal = tbl.M?.[g]?.[k]
        const fVal = tbl.F?.[g]?.[k]
        row.push(mVal !== undefined ? mVal : '')
        row.push(fVal !== undefined ? fVal : '')
      }
      rows.push(row)
    }

    rows.push([])
  }

  return rows
}

// ── Main ───────────────────────────────────────────────────────────────────────

const wb = XLSX.utils.book_new()

for (const sheetDef of SHEETS) {
  const data = buildSheet(sheetDef)
  const ws   = XLSX.utils.aoa_to_sheet(data)

  // Larghezze colonne
  ws['!cols'] = [{ wch: 12 }, ...Array(30).fill({ wch: 10 })]

  XLSX.utils.book_append_sheet(wb, ws, sheetDef.name)
}

const outFile = resolve(ROOT, 'tabelle_riferimento_test.xlsx')
XLSX.writeFile(wb, outFile)
console.log(`✅  Generato: ${outFile}`)
