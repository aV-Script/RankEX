...
import { CATEGORY_TESTS, CATEGORIE } from '../../constants'
...
export function TestGuidePage() {
  ...
  const [selectedTest, setSelectedTest] = useState(CATEGORY_TESTS.health[0].key)
  const [menuOpen,     setMenuOpen]     = useState(false)

  const tests       = CATEGORY_TESTS[selectedCat] ?? CATEGORY_TESTS.health
  const guide       = TEST_GUIDES[selectedTest]
  const catColor    = CATEGORY_COLORS[selectedCat]
  const catDef      = CATEGORIE.find(c => c.id === selectedCat)

  const handleSelectCat = (catId) => {
    setSelectedCat(catId)
    setSelectedTest(CATEGORY_TESTS[catId][0].key)
    setMenuOpen(false)
  }
  ...
  return (
    <div className="text-white">
      ...
      <div className="flex min-h-[calc(100vh-113px)] lg:min-h-[calc(100vh-57px)]">
        ...
        {/* ── Sidebar desktop ── */}
        <aside className="hidden lg:flex w-60 xl:w-72 shrink-0 border-r border-white/[.05] flex-col sticky top-0 h-screen overflow-y-auto">
        ...
        {/* ── Mobile: selettori a tendina ── */}
        <div className="lg:hidden w-full">
          <div className="px-4 py-3 border-b border-white/[.05] flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              {CATEGORIE.map(cat => (
                <button key={cat.id}
                  onClick={() => handleSelectCat(cat.id)}
                  className="flex-1 py-2 rounded-xl font-display text-[10px] tracking-widest cursor-pointer border transition-all"
            ...
            >
              <div className="text-left">
                ...
                <span className="font-body text-[14px] text-white mt-0.5 block">
                  {tests.find(t => t.key === selectedTest)?.label} — {guide?.name}
                </span>
              </div>
          ...
          {guide && <div className="px-4 py-6"><GuideContent guide={guide} color={catColor} /></div>}
        </div>

        {/* ── Contenuto desktop ── */}
        {guide && (
          <main className="hidden lg:block flex-1 px-8 py-8 max-w-3xl overflow-y-auto">
            <GuideContent guide={guide} color={catColor} />
          </main>
        )}
      </div>
    </div>
...
function GuideContent({ guide, color }) {
  return (
    <>
      <div className="mb-8">
        <p className="font-display text-[10px] tracking-[3px] mb-2" style={{ color }}>GUIDA ESECUZIONE TEST</p>
        <h2 className="font-display font-black text-[28px] lg:text-[32px] text-white m-0 leading-tight">
          {guide.name}
        </h2>
        <div className="flex gap-2 mt-3 flex-wrap">
          <Pill label="Statistica" value={guide.stat}     color={color} />
          <Pill label="Unità"      value={guide.unit}     color={color} />
          <Pill label="Durata"     value={guide.duration} color={color} />
        </div>
      </div>

      <GuideSection title="Attrezzatura necessaria" color={color}>
      ...
      <GuideSection title="Riscaldamento" color={color}>
      ...
      <GuideSection title="Protocollo di esecuzione" color={color}>
        <ol className="m-0 pl-0 list-none flex flex-col gap-4">
          {guide.protocol.map((step, i) => (
            <li key={i} className="flex gap-4 items-start">
              <span className="font-display font-black text-[13px] w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5"
      ...
      <GuideSection title="Note operative" color={color}>
        <ul className="m-0 pl-0 list-none flex flex-col gap-2">
          {guide.notes.map((note, i) => (
            <li key={i} className="flex gap-2.5 items-start rounded-xl px-3 py-2.5"
              style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <span className="mt-0.5 shrink-0 text-[12px]" style={{ color }}>!</span>
      ...
      <div className="mt-6 rounded-2xl p-6 flex flex-col items-center justify-center gap-2"
...
function GuideSection({ title, children, color }) {
  return (
    <div className="mb-8">
      <div className="font-display text-[10px] tracking-[3px] uppercase mb-4"
...
function Pill({ label, value, color }) {
  return (
    <div className="flex items-center gap-1.5 rounded-lg px-3 py-1.5"
      ...
      <span className="font-display text-[11px]" style={{ color }}>{value}</span>
    </div>
  )
}
