 ...
 *   - 1 organizzazione soccer_academy
 *   - 2 membri (org_admin + trainer) — solo documenti Firestore, no Auth
 *   - 24 giocatori (2 GK · 8 DEF · 8 MID · 6 FWD)
 *   - 10 campionamenti mensili per giocatore (ultimi 10 mesi)
 *
 * Utilizzo:
 *   1. Accedi all'app come super_admin (necessario per creare l'org)
 *   2. Apri DevTools → Console
 *   3. L'app espone window.__seedSoccer in dev mode (vedi App.jsx o dove preferisci)
 *   4. Esegui: await window.__seedSoccer()
 *   5. Prendi nota dell'orgId stampato in console → usalo per il login del trainer
 *
 * Oppure importa e chiama direttamente da un componente dev.
 */
...
// ── Roster (24 giocatori) ─────────────────────────────────────────────────────
// variation: offset ±8 applicato uniformemente a tutte le stat del profilo ruolo
const PLAYERS = [
  // 2 portieri
...
// Genera campionamenti mensili (10 mesi): restituisce array newest-first
function generateCampionamenti(ruolo, variation) {
  const profile = ROLE_PROFILES[ruolo]

  // Stat obiettivo (mese 10 — più recente)
  const targetStats = {}
  ...
  for (let monthAgo = 9; monthAgo >= 0; monthAgo--) {
    ...
    const progress = (9 - monthAgo) / 9  // 0 (oldest) → 1 (newest)
    const stats = {}
  ...
  camps.reverse() // newest first (campionamenti[0] = latest)

  // Calcola XP totale da campionamenti (più ~2000 XP simulati da sessioni)
  let campXP = 0
  ...
  const totalXP = 2000 + campXP  // simula ~10 mesi di sessioni
...
export async function seedSoccer({
  orgName    = 'ASD Calcio Demo',
  createOrg  = true,
  orgId      = `soccer-demo-${Date.now().toString(36)}`,
} = {}) {
  console.log(`🌱 Avvio seed soccer — orgId: ${orgId}`)
  ...
  // ── Membri (senza Auth reale — solo Firestore) ──────────────────────────────
  const MEMBERS = [
    {
      uid:   'seed-admin-uid',
      role:  'org_admin',
      name:  'Admin Demo',
      email: 'admin@soccer-demo.it',
    },
    {
      uid:   'seed-trainer-uid',
      role:  'trainer',
      name:  'Coach Demo',
      email: 'coach@soccer-demo.it',
    },
  ]

  MEMBERS.forEach(m => {
    const memberRef = doc(db, `organizations/${orgId}/members`, m.uid)
    batch.set(memberRef, {
      role:     m.role,
      name:     m.name,
      email:    m.email,
  ...
  console.log(`     Crea un account reale e copia il suo UID`)
  console.log(`  2. Aggiorna il documento /users/{uid} con:`)
